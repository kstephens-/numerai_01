...
for ind, (train_index, test_index) in enumerate(folds):
    ...
    #test_pred_cal = ir.transform(test_pred.reshape(-1, 1))

    ensemble_train.loc[train_test.index, 'svm'] = train_test_pred
...
final_submission = pd.DataFrame({'t_id': submission['t_id'].values,
                                 'probability': sub})
final_submission.to_csv('../submissions/{}_v{}.csv'.format('svm', version), index=False)

ensemble_train.to_csv('../data/ensemble_train/{}_v{}.csv'.format('svm', version), index=False)
